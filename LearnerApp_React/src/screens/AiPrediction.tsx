...
export default function AiPrediction() {
  ...
  const [prediction, setPrediction] = useState<PredictionResponse>({
    ...
    status: 'Placement Ready',
    recommendation: 'Load a learner ID to generate the latest prediction.',
  });
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const radialData = [{ name: 'Placement', value: prediction.placementProbability, fill: '#89b4fa' }];
  ...
  const handlePredict = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!learnerId.trim()) {
      setError('Enter a learner ID before generating a prediction');
      return;
    }
    ...
    try {
      const response = await apiRequest<unknown>(`/api/predict/${learnerId.trim()}`, {
        method: 'POST',
      });
    ...
    <motion.div variants={cV} initial="hidden" animate="visible" className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 py-10 lg:py-14">
      ...
      <motion.div variants={iV} className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
        <Card>
          ...
          <div className="flex flex-wrap sm:flex-nowrap items-center gap-4 sm:gap-5 mb-10 p-4 sm:p-5 lg:p-6 bg-base/50 rounded-2xl border border-surface0/40">
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br from-blue/20 to-mauve/20 border border-blue/20 text-blue text-base sm:text-lg font-bold flex items-center justify-center shrink-0">RS</div>
            <div className="flex-1 min-w-0">
              <div className="text-base sm:text-lg font-bold truncate text-text">Rahul Sharma</div>
              <div className="text-[10px] sm:text-xs text-overlay0 font-mono mt-1 truncate">Selected learner snapshot</div>
            </div>
            <Badge variant="blue" className="shrink-0 whitespace-nowrap">Batch: Active</Badge>
          </div>
          <form onSubmit={handlePredict} className="mb-10">
            <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-4 items-end">
              <div>
                <label className="block text-[11px] font-semibold text-subtext0 mb-3 tracking-wider uppercase font-mono">Learner ID</label>
                <input type="number" value={learnerId} onChange={(event) => setLearnerId(event.target.value)} className="w-full px-5 py-3.5 bg-base border border-surface0/60 rounded-xl text-text text-sm placeholder:text-overlay0 transition-all" placeholder="1" />
              </div>
              <motion.button type="submit" whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.97 }} disabled={isLoading}
                className="w-full sm:w-auto py-3.5 px-6 rounded-xl text-sm font-bold bg-gradient-to-r from-blue to-lavender text-crust cursor-pointer flex items-center justify-center gap-2.5 shadow-[0_4px_24px_rgba(137,180,250,0.25)] disabled:cursor-not-allowed disabled:opacity-70">
        ...
        <div className="bg-gradient-to-br from-blue/8 to-mauve/8 border border-blue/15 rounded-2xl p-6 sm:p-8 lg:p-10">
          <div className="text-center mb-10">
            ...
            <div className="relative inline-block mx-auto">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <motion.div className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold text-blue"
                  initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                  transition={{ duration: 0.5, type: 'spring' }}>{prediction.placementProbability}%</motion.div>
              </div>
            </div>
          ...
          <div className="bg-peach/8 border border-peach/15 rounded-2xl p-6 lg:p-7 mb-10">
            ...
            <p className="text-sm text-text leading-relaxed">
              {prediction.recommendation || 'Load a learner ID to surface the recommendation returned by Spring and Python.'}
            </p>
          </div>

          <div className="rounded-2xl border border-surface0/40 bg-base/30 px-5 py-4 text-sm text-subtext0 leading-relaxed">
            Choose a learner ID to refresh the result and see the latest placement signal.
          </div>
        </div>
      </motion.div>
    </motion.div>
